...
def pct_fragments_in_promoters(adata, feature, gtf_file=None, bam_file=None, fragments_file=None,
    ...
    if not gtf_file and not species:
        ...
        features_gtf = gtf_file
        
    pct_fragments_overlap(adata, regions_file=gtf_file, bam_file=bam_file, fragments_file=fragments_file,
                              cb_col=cb_col, regions_name=feature, nproc=nproc, sort_bam=sort_bam)

def pct_fragments_in_features(adata, input_dir=None, bam_file=None, fragments_file=None,
                               cb_col=None, species=None, nproc=1, sort_bam=False):
    """
    ...
    # gets features from input file depending on *.FEATURE.gtf format
    out_content = os.listdir(input_dir)
    ...
    for file in out_content:
        if file.endswith('.gtf'):

            # call function
            feature_counter += 1
            print(file)
            pct_fragments_overlap(adata, regions_file=input_dir + file, bam_file=bam_file, fragments_file=fragments_file,
                              cb_col=cb_col, regions_name=feature_list[feature_counter], nproc=nproc, sort_bam=sort_bam)
...
def pct_fragments_overlap(adata, regions_file, bam_file=None, fragments_file=None, cb_col=None,
    ...
    if not overlap_file:
        print("There was no overlap for: " + regions_file)
        return
    ...
    if 'n_total_fragments' in adata.obs:
         fragments_df.drop([col_total_fragments], axis=1, inplace=True)
