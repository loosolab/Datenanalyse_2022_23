...
through the total number of reads for every cell barcode. To use this function you need the [calc_overlap_pct.py](/wp2/src/calc_overlap_pct.py)
, from the sctoolbox, prepared inside the [src](/wp2/src)-Folder in this repository.
...
After all preperation has been done, the final step is to look at data. To overcome the shear volume of data, we prepared different presentation styles, they are all located inside `gff_analyser/atacFeatureGraphs.py` and may be extended there. 

#### Different Presentation Styles

The violin plots are helpful to quickly assess if there are plenty of cells in a dataset, furthermore it displays the mean and their dispersion. This can be used to compare the overall spread of featurerate per cell. One function that we implemented `compare_feature_to_celltypes` compares, as the name suggests, one feature against all celltypes. If you found celltypes that have virtue, you can compare them in greater detail by firstly calling `compare_feature_to_celltype` taking nearly the same parameters but limiting the output violins to the base values of all cells and the specific celltype.
...
<p align="center">
   <img src="docs/images/dimension_intestinal_2.png" />
</p>
...
```py
def compare_feature_to_celltypes(adata, feature: list or str, comparator: str, max_size, name, out=None, sharey=True, rotation):
```
...
key | String or List of Strings | The key or keys (features) that are displayed
comparator | String or List of Strings | The key or keys (Cell Type) that are used to compare the keys to
name | String | The Y-Label that should be attached to the plot
out | String (optional) | The location that the figures are saved to
groups | List of Strings (optional) | a list of values out of `comparator` that are worth extra attention
rotation | Int (optional, default=0) | Rotate the x-label for the plots, most likely to be 90 or 0.

<p align="center">
   <img src="docs/images/violin_plots_large.png" />
</p>

<p align="center">
   <img src="docs/images/violin_plots.png" />
</p>
...
Following that are multiple functions to display the data in various ways, they should be more or less self explanatory, or explained in this [section](#data-presentation)

There are a few functions to help you filter your data, which isn't necessary if you are using the highly processed _catlas_ object.

To run dimensionreductions you will need to normalize your data, calculate PCA and neighbours, calculate the UMAP and finally cluster your data using the _leiden algorithm_.

Annotation is not required if you are using catlas files, but if you want to try your hands, use the uropa snippets supplied.

To aid in the quick creation of plots you can generate lists of various features and use those in combination to the different plotting function discussed earlier.
